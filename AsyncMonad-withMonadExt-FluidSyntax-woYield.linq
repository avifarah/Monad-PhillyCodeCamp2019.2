...
static class MonadExt
{
	public static Func<IEnumerable<TSrc>> LiftToEnum<TSrc>(Func<TSrc> func)
	{
		return () => new List<TSrc> { func() };
	}
}
...
class Program
{
	static void Main()
	{
		...
		sw.Restart();
		var tsks = (
			ppl.SelectMany(person => MonadExt.LiftToEnum(() => Task.Run(() => person.CalcCost())
						.Select(pCost => (person, pCost, Thread.CurrentThread.ManagedThreadId))
					)()
				)
		);
		var tskList = tsks.ToList();
		...
		foreach (var tsk in tskList.WaitAllOneByOne())
		{
